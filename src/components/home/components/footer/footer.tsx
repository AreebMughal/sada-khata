      ...
      <div className="mx-auto max-w-2xl mt-64 pb-16 px-4 sm:px-6 lg:max-w-7xl lg:px-8">
        <div className="mt-24 grid grid-cols-1 gap-y-10 gap-x-16 sm:grid-cols-2 lg:grid-cols-12 xl:gap-x-8">
          ...
          <div className="col-span-6">
            <img className="block h-12 w-20px mb-4" src={'/images/Logo/logo.svg'} alt="Crypto-Logo" />
          ...
          {products.map(product => (
            <div key={product.id} className="group relative col-span-2">
              <p className="text-xl font-medium mb-9">{product.section}</p>
              <ul>
                {product.link.map((link: string, index: number) => (
                  <li key={index} className="mb-5">
                    <Link href="/" className=" text-sm font-normal mb-6 space-links">
                      {link}
                    </Link>
                  </li>
                ))}
          ...
          <div className="col-span-4">
            <h3 className="text-xl font-medium mb-9">Contact Us</h3>
            <h4 className="text-sm font-normal mb-6 flex gap-2">
